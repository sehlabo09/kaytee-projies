...
export default function Report() {
  ...
  return (
    <div className="page">
      ...
      {topProducts.length === 0 ? <p>No sales</p> :
        <ul>{topProducts.map(([name, qty], i) => <li key={i}>{name} - {qty} sold</li>)}</ul>
      }

      <h2>Transactions</h2>
      {filteredTx.length === 0 ? <p>No transactions today.</p> :
        <table className="table">
          <thead><tr><th>Product</th><th>Qty</th><th>Price</th><th>Subtotal</th><th>Time</th></tr></thead>
          <tbody>
            {filteredTx.map(tx => (
              <tr key={tx.id}>
                <td>{tx.name}</td>
